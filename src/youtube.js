...
const fetchVideos = async account => {
  ...
  if (!account.youtubeId) {
    return account;
  }
  const since = moment()
    .subtract(process.env.STATS_SINCE_DAYS, "days")
    .utc()
    .format();
  const maxResults = process.env.LAST_VIDEOS_ANALIZED;
  await request(
    `https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${account.youtubeId}&order=date&type=video&publishedAfter=${since}&maxResults=${maxResults}&key=${process.env.API_KEY}`
  )
    .then(body => {
      const results = JSON.parse(body);
...
const fetchVideoStats = async video => {
  ...
  await request(
    `https://www.googleapis.com/youtube/v3/videos?part=statistics&id=${video.id.videoId}&key=${process.env.API_KEY}`
  )
    .then(stats => {
