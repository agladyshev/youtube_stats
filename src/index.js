...
const getYoutubeProfiles = async (req, res, next) => {
  ...
  Promise.all(accounts.map(youtube.fetchYoutubeProfile)).then(
    updatedAccounts => {
...
const getVideoStats = async account => {
  ...
  if (Array.isArray(account.videos) && account.videos.length) {
    updatedAccount.videos = await Promise.all(
      account.videos.map(youtube.fetchVideoStats)
    );
  }
...
const calculateStats = async (req, res, next) => {
  ...
  res.body = await Promise.all(
    accounts.map(account => {
      const updatedAccount = account;
      if (Array.isArray(account.videos) && account.videos.length) {
        updatedAccount.statistics = account.videos.reduce(
          (accumulator, video, index) => {
            ...
            return {
              ...
              dislikesRecent:
                accumulator.dislikesRecent + parseInt(dislikeCount, 10),
              commentsRecent:
                accumulator.commentsRecent + parseInt(commentCount, 10),
              favoritesRecent:
                accumulator.favoritesRecent + parseInt(favoriteCount, 10),
              videosRecent: index
            };
          },
          {
        ...
        updatedAccount.youtubeVideoStatus =
          account.youtubeVideoStatus || "Not available";
      }
      return updatedAccount;
    })
  );
...
app.get("/profiles", getYoutubeProfiles, updateProfiles, (req, res) =>
  res.sendStatus(200)
);
app.get(
  "/videos",
...
app.listen(port, () =>
  console.log(`YouTube module is listening on port ${port}!`)
);
